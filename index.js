async function moduleProject4() {
  ...
  // 👉 Tasks 1 - 5 go here
  // Task 1:
  // const weatherDisplay = weatherWidget.style.display = "none"
    ...
    .addEventListener("change", async (evt) => {
      ...
      try {
        ...
        document.querySelector("#todayDescription").textContent =
          descriptions.find(
            (d) => d[0] === data.current.weather_description
          )[1];
        document.querySelector(
        ...
        data.forecast.daily.forEach((day, idx) => {
          ...
          // day.date.moment('2023-05-25', '2023-05-26', '2023-05-27').format('dddd') (To fix the date formatting maybe?)
          // Detroit and the temperatires still end up being hard coded with the above added.
          apparent.textContent = descriptions.find(
            (d) => d[0] === day.weather_description
          )[1];
        ...
        document.querySelector("#location").firstElementChild.textContent =
          data.location.city;
      } catch (error) {
        console.log(
          "Promise rejected with an error.message -->",
          error.message
        );
      }
    });
